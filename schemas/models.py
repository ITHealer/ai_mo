...
# app/models.py (Thêm các models mới cho job system)
"""
Pydantic models and enums for content moderation system.
"""
from enum import Enum
from typing import List, Optional
...
class VideoModerationRequest(BaseModel):
    ...
    video_url: Optional[str] = Field(None, description="URL to video file")
    sampling_fps: float = Field(default=0.5, ge=0.1, le=2.0, description="Frame sampling rate")
    use_keyframes: bool = Field(default=False, description="Use keyframe detection")
...
class KeyframeMethod(str, Enum):
    ...
    UNIFORM = "uniform"  # Uniform sampling (no keyframe detection)
    DIFFERENCE = "difference"  # Frame difference analysis
    SCENE = "scene"  # Scene detection (PySceneDetect)


class VideoModerationRequest(BaseModel):
    """Request model for video moderation"""
    video_url: Optional[str] = Field(None, description="URL to video file")
    sampling_fps: float = Field(default=0.5, ge=0.1, le=2.0, description="Frame sampling rate (for uniform mode)")
    keyframe_method: KeyframeMethod = Field(default=KeyframeMethod.UNIFORM, description="Keyframe detection method")
    
    # PySceneDetect parameters (only used when keyframe_method=SCENE)
    scene_detector_type: str = Field(default="content", description="Scene detector: 'content' or 'adaptive'")
    scene_threshold: float = Field(default=27.0, ge=1.0, le=100.0, description="Scene detection sensitivity")
    min_scene_length: float = Field(default=1.0, ge=0.1, le=10.0, description="Minimum scene length in seconds")